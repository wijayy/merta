...
<flux:with-field :$attributes :$name>
    <div {{ $attributes->only('class')->class('w-full relative block group/input') }} data-flux-input>
        ...
        <input type="{{ $type }}" {{-- Leave file inputs unstyled... --}}
            ...
            <?php if (isset($name)): ?> name="{{ $name }}" <?php endif; ?> <?php if ($maskDynamic): ?>
        x-mask:dynamic="{{ $maskDynamic }}" @elseif ($mask) x-mask="{{ $mask }}"
            <?php endif; ?> <?php if (is_numeric($size)): ?> size="{{ $size }}" <?php endif; ?>
            @unblaze(scope: ['name' => $name ?? null, 'invalid' => $invalid ?? false]) <?php if ($scope['invalid'] || ($scope['name'] && $errors->has($scope['name']))): ?> aria-invalid="true" data-invalid <?php endif; ?>
            @endunblaze data-flux-control data-flux-group-target <?php if($loading): ?>
            wire:loading.class="{{ $inputLoadingClasses }}" <?php endif; ?> <?php if($loading && $wireTarget): ?>
            wire:target="{{ $wireTarget }}" <?php endif; ?>>
